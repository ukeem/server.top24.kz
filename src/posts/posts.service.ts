...
export class PostsService {
    ...
    private async getCategoryId(query: string) {
        ...
        const prompt = `
			...
			Если запрос не подходит ни к одной категории или список категорий пуст, придумай наиболее подходящее название категории (Одним словом).  
			Ответ должен содержать **только одно слово** — название категории (Одним словом), **без пояснений и комментариев**.  
    ...
    private async rewriteContent(query: string, content: string) {
        ...
        const prompt = `
			...
			**Длина вывода:** Мвксимально развернуто, приближенно к 15000 токенам.\n\n
